...
[.component,class=step]
--
For a link object with a `+channel+` property defined starting with `+origin/a/wis2+` or `+cache/a/wis2+`, check that the fourth token of the `+channel+` property (split by `+/+`) is equal to the fourth token of the WCMP2 identfier (`id`) property (split by `+:+`).
--
