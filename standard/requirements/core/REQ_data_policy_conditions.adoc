...
^|C |The `+properties.wmo:dataPolicy.additionalConditions.scheme+` property SHALL be used to identify a codelist or controlled vocabulary to which the condition name applies.
^|D |If there are *no* additional conditions, the fixed value of `+none+` SHALL be provided `+properties.wmo:dataPolicy.additionalConditions.name+` without any scheme identified.
|===
