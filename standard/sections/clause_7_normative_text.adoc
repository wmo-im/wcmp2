...
include::../recommendations/core/REC_title.adoc[]
...
A WCMP record can include links to real-time notifications via WIS2.  A typical MQTT link in a WCMP document is defined as follows:
...
In some cases, a dataset may be organized in a manner which requires additional "filtering" for
data consumers that are only interested in a specific subset of the data granules advertised by the WIS2 notifications.  Some examples include (but are not limited to):
...
The WCMP link advertising the MQTT channel can be extended to describe applicable filters that map to the structure of a `properties` object in the associated WIS2 Notification Messages.

.Example: Surface weather observations: WCMP MQTT link with a `filters` object
[source,json]
----
{
  "rel"  : "items",
...
.Example: Numerical weather prediction: WCMP MQTT link with a `filters` object
[source,json]
----
{
  "rel"  : "items",
