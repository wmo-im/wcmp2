...
The WCMP record schema is based on the Requirements Class "Record Core (Common Component)" of OGC API – Records – Part 1: Core schema and the associated information model. WCMP records compliant with the WCMP schema are therefore compliant with the OGC API – Records record schema.
...
===== 1.19.6 Client side filters

For data available on WIS2 via MQTT, a WCMP record provides linkages to the WIS2 Global Broker.

A typical MQTT link in a WCMP document is defined as follows:

.Example: Typical WCMP MQTT link
[source,json]
----
{
  "rel"  : "items",
...
In some cases, a dataset may be organized in a manner which requires additional "filtering" such that a
data consumer is only interested in a certain subset of the data granules being advertised by a given WIS2 Notification.  Some examples include (but are not limited to), where a data consumer may be only be interested in:
...
A WCMP link can be extended to advertise applicable filters that map to the structure of a WIS2 Notification Message `properties` object.

.Example: Surface weather observations: WCMP MQTT link with additional properties
[source,json]
----
{
  "rel"  : "items",
...
.Example: Surface weather observations: WNM properties
[source,json]
----
{
  "properties": {
...
When implemented in a WCMP link object, a data consumer can then:
...
.Example: Numerical weather prediction: WCMP MQTT link with additional properties
[source,json]
----
{
  "rel"  : "items",
  ...
  "filters": {
    ...
    "forecast_hour": {
       ...
       "format": "date-time",
       "title": "Forecast hour",
       "example": "2025-05-27T03:00:00Z"
    }
  }
}
----

.Example: Numerical weather prediction: WNM additional properties
[source,json]
----
{
  "properties": {
    "model_run": "2025-05-27T00:00:00Z",
    "forecast_hour": "2025-05-27T03:00:00Z"
  }
}
----
...
- subscribe to real-time notifications to the given topic
- perform client side filtering against all incoming WNMs with `properties.model_run = "2025-05-27T00:00:00Z" and properties.forecast_hour = "2025-05-27T03:00:00Z"`

include::../recommendations/core/REC_client_side_filters.adoc[]
