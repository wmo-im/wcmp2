...
|``properties.status``
|optional
|The operational status of a given dataset (see <<_properties_status>>)
  ...
  "themes": [
    {
      "concepts": [{
        "id": "weather",
        "url": "https://github.com/wmo-im/wis2-topic-hierarchy/blob/main/topic-hierarchy/earth-system-discipline/index.csv#L2"
      }],
...
[NOTE]
This property relates to the date of the metadata record, not the start or end time of a given dataset.  See <<_temporal_extent>> for more information on defining dataset temporal extents.
