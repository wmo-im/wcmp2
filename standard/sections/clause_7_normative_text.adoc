...
|``links``
|**required**
//q: do offline linkages exist too? 
...
Licensing and copyright are expressed via the links property (see distribution section), providing access, license and attribution details as required. [FIX: Conditions on use are essential for the WMO Unified Data Policy], but for transparency and clarification any other conditions should also be indicated.
...
It is useful to add provider-specific details to complete information about data policy and additional conditions.
...
The `+links+` property describes URLs and APIs for accessing or subscribing to the dataset.  Links are a core
mechanism for further interacting with data, and provide "actionable" workflow to enhance and improve the
user experience with "less clicks" to subscribe to or download data.

The `+links+` property may also describe URLs for related documentation, visual images of the dataset or related data.
...
|The relationship that the link bears in the context of the WCMP record.  This is a controlled vocabulary defined
 by IANA link relations footnote:[https://www.iana.org/assignments/link-relations/link-relations.xhtml] or WCMP2
 defined extensions footnote:[https://github.com/wmo-im/wcmp2-codelists/blob/main/codelists/link-type.csv] (see <<_link_relation_selection>>)
...
|If the link is a URI template (``templated=true``), the variable definitions that help the user construct a
 link from the template with the appropriate values
...
|``updated``
|**optional**
|The date that the link was updated
...
|`http://def.wmo.int/def/rel/wmdr/-/FacilitySet`

|A link to a single station that the dataset is based on
|`http://def.wmo.int/def/rel/wmdr/-/EnvironmentalMonitoringFacility`
