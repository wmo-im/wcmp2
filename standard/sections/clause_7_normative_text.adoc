...
|**Conditional**
|Classification code of ``core`` or ``recommended`` based on the WMO Unified Data Policyfootnote:[https://library.wmo.int/idurl/4/58009].  **Required** for datasets (see <<_1_18_properties_wmo_data_policy>>)
...
The `+wmo:dataPolicy+` property is a codelist that identifies the classification of the dataset exchange as described by the WMO Data Policy (Resolution 1 (Cg-Ext(2021)))footnote:[https://library.wmo.int/idurl/4/58009] for the international exchange of Earth system data. The codelist values are `+core+` or `+recommended+`.  The `+wmo:dataPolicy+` property is required if the metadata record describes a dataset.
...
|The relationship that the link bears in the context of the WCMP record.  This is a controlled vocabulary defined
 by link:https://www.iana.org/assignments/link-relations/link-relations.xhtml[IANA link relations] or link:https://codes.wmo.int/wis/link-type[WIS link type] extensions. (see table <<_link_relation_selection>>)
...
|``security``
|Optional
|For access controlled data, a description of the access control mechanism applied (see <<1_19_4_access_control>>)
