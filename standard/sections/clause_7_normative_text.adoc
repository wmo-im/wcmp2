...
datasets and their associated services.
...
temporal extents) via the `+additionalExtents+` property.  The `+additionalExtents.spatial.bbox+` property
...
A WCMP record can communicate additional time instants or periods via the `+additionalExtents+` item property.
Temporal extents can be fully bound or open in either direction (e.g., until or before).  A `+additionalExtents.temporal.resolution+`
...
A persistent (or handle) identifier is used to provide a long lasting reference to a digital resource.  Persistent identifiers
are commonly used in scientific publications.
