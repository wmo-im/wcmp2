...
In addition to the presence of the graphic overview image it would also be valuable to provide consistent image dimensions (e.g. 800x800 pixels) such that all images are normalized so the scaling and alignment of these images can be applied consistently by web applications rendering search results.
